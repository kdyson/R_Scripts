...
extract.titan.taxa <- function(titan.out, p.val.cutoff = 0.05, taxa.label.name = "Genus"){
    titan.out <- titan.out
    ...
    colnames(titan.out.taxonomy.summary)[1:ncol(titan.out.taxonomy.summary)] <-
        c("Decreasing/Increasing Z Taxa",
          taxa.label.name,
          "Mean Z Score",
          "Mean Purity",
          "Mean Reliability",
...
extract.titan.taxa.NGS <- function(titan.out, taxonomy.table, taxonomy.table.merge = "taxa.label.unique", taxa.level = Genus, p.val.cutoff = 0.05, taxa.label.name = "taxa.label.unique" ){
    titan.out <- titan.out
    ...
    titan.out.p.val <-
        titan.out.p.val[order(titan.out.p.val$zscore, decreasing = T),]
    titan.out.p.val <-
        merge(titan.out.p.val,
              taxonomy.table,
              by.x = taxa.label.name, by.y = taxonomy.table.merge,
              sort = F) %>%
    ...
    colnames(titan.out.taxonomy.summary)[1] <- "Decreasing/Increasing Z Taxa"
...
cell.filler <- function(titan.taxa.tables, which.z, taxon.list, taxa.col.name, table.col.names, z.score.pct.cutoff, round.val){
...
for (i in 1:length(titan.taxa.tables)) {
    ...
    temp.table <- temp.table[ temp.table$`Decreasing/Increasing Z Taxa` == which.z , ] 
        ...
        if (taxon.list[j] %in% check.list) {
            ...
            temp.row <-
                temp.table[select(temp.table,!!taxa.col.name) == taxon.list[j] &
                               !(is.na(select(temp.table,!!taxa.col.name))),]
            
            # bold the z score if it's above the cutoff
            z.score.text <-
                ifelse(
                    temp.row[, "Mean Z Score"] >= z.score.percentile ,
                    yes = paste0("**", temp.row[, "Mean Z Score"] %>% round(digits = round.val), "**"),
...
combine.titan.results <- function(titan.taxa.tables, taxa.col.name, table.col.names, z.score.pct.cutoff = .5, round.val = 3){
...
# make that only uniques, and remove NA
    ...
    all.z1.taxon <- all.z1.taxon[!(is.na(all.z1.taxon))] %>% unique()
    all.z1.taxon <- all.z1.taxon[order(all.z1.taxon)]
    
    
    all.z2.taxon <- all.z2.taxon[!(is.na(all.z2.taxon))] %>% unique()
...
# smash the two resulting tables together
    ...
    summary.table <- summary.table[ order(summary.table$`All Taxon`), ]
